...
const toTransaction = (topup: TopupResponse): Transaction => ({
  id: topup.id,
  date: topup.created_at,
  description: topup.kind === "sms"
    ? `SMS topup: ${topup.sms_count.toLocaleString()} credits${topup.phone ? ` from ${topup.phone}` : ""}`
    : `Airtime purchase: ${topup.phone || "recipient"}${topup.network ? ` (${topup.network})` : ""}`,
  amount: topup.amount,
  status: topup.status === "failed" ? "Failed" : topup.status === "pending" ? "Pending" : "Paid",
});
...
export default function BillingPage() {
  ...
  const filteredAndSortedTransactions = useMemo(() => {
    ...
    return result;
  }, [transactions, filter, dateRange, sortField, sortDirection]);
                ...
                {isLoading ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-12">
                      ...
                      <p className="text-xs text-muted-foreground">Loading billing transactions</p>
                    </TableCell>
                  </TableRow>
                    ...
                    <TableCell colSpan={6} className="text-center py-12">
                      <div className="flex flex-col items-center justify-center space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground max-w-xs">
                          {loadError || "Try adjusting your date filters or range"}
                        </p>
                      </div>
                    </TableCell>
                  </TableRow>
